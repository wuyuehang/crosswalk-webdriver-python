...
class DeviceManager(object):
  ...
  def AcquireDevice(self):
    ...
    for it in devices:
      if self.IsDeviceLocked(it):
        ...
        status = Status(kOk)  
        break
    #VLOG(0, "acquire device active devices: " + str(self.active_devices))
  ...
  def AcquireSpecificDevice(self, device_serial):
    VLOG(0, "lol... trapped into AcquireSpecificDevice")
    devices = []
    status = self.xdb.GetDevices(devices)
    if status.IsError():
      return status, None
...
if __name__ == "__main__":
  ...
  dm_test = DeviceManager(adb)
  status = dm_test.AcquireDevice(device)
  print ">>>>>>>> AcquireDevice : %s" % status.Message() 

  status = dm_test.AcquireSpecificDevice("emulator-5554", device)
  print ">>>>>>>> AcquireSpecificDevice : %s" % status.Message() 
